...
enum TransMode
{
	...
	ViewEye = 4,
	ViewUp = 5,
	// add light editing mode and shininess editing mode
	LightEdit = 6,
	ShininessEdit = 7,
};
...
int per_vertex_or_per_pixel = 0;
int eye_status[7];
float shininess;
GLuint mag_mode, min_mode;

// end of self-defined params
...
void RenderScene(int per_vertex_or_per_pixel) {	
	...
	temp_info = glGetUniformLocation(program, "light_src");
	glUniform1i(temp_info, light_src);

	temp_info = glGetUniformLocation(program, "shininess");
	glUniform1f(temp_info, shininess);

	temp_info = glGetUniformLocation(program, "light_info[0].position");
	glUniform3f(temp_info, light_info[0].position.x, light_info[0].position.y, light_info[0].position.z);
	temp_info = glGetUniformLocation(program, "light_info[0].direction");
	glUniform3f(temp_info, light_info[0].direction.x, light_info[0].direction.y, light_info[0].direction.z);
	temp_info = glGetUniformLocation(program, "light_info[0].diffuse");
	glUniform3f(temp_info, light_info[0].diffuse.x, light_info[0].diffuse.y, light_info[0].diffuse.z);

	temp_info = glGetUniformLocation(program, "light_info[1].position");
	glUniform3f(temp_info, light_info[1].position.x, light_info[1].position.y, light_info[1].position.z);
	temp_info = glGetUniformLocation(program, "light_info[1].direction");
	glUniform3f(temp_info, light_info[1].direction.x, light_info[1].direction.y, light_info[1].direction.z);
	temp_info = glGetUniformLocation(program, "light_info[1].diffuse");
	glUniform3f(temp_info, light_info[1].diffuse.x, light_info[1].diffuse.y, light_info[1].diffuse.z);

	temp_info = glGetUniformLocation(program, "light_info[2].position");
	glUniform3f(temp_info, light_info[2].position.x, light_info[2].position.y, light_info[2].position.z);
	temp_info = glGetUniformLocation(program, "light_info[2].direction");
	glUniform3f(temp_info, light_info[2].direction.x, light_info[2].direction.y, light_info[2].direction.z);
	temp_info = glGetUniformLocation(program, "light_info[2].diffuse");
	glUniform3f(temp_info, light_info[2].diffuse.x, light_info[2].diffuse.y, light_info[2].diffuse.z);
	temp_info = glGetUniformLocation(program, "light_info[2].spot_cutoff");
	glUniform1f(temp_info, light_info[2].spot_cutoff);
	temp_info = glGetUniformLocation(program, "light_info[2].spot_exp");
	glUniform1f(temp_info, light_info[2].spot_exp);

	temp_info = glGetUniformLocation(program, "light_info[1].att_con");
    glUniform1f(temp_info, light_info[1].att_con);
    temp_info = glGetUniformLocation(program, "light_info[1].att_lin");
    glUniform1f(temp_info, light_info[1].att_lin);
    temp_info = glGetUniformLocation(program, "light_info[1].att_qua");
    glUniform1f(temp_info, light_info[1].att_qua);

    temp_info = glGetUniformLocation(program, "light_info[2].att_con");
    glUniform1f(temp_info, light_info[2].att_con);
    temp_info = glGetUniformLocation(program, "light_info[2].att_lin");
    glUniform1f(temp_info, light_info[2].att_lin);
    temp_info = glGetUniformLocation(program, "light_info[2].att_qua");
    glUniform1f(temp_info, light_info[2].att_qua);

	temp_info = glGetUniformLocation(program, "per_vertex_or_per_pixel");
	glUniform1i(temp_info, per_vertex_or_per_pixel);
	
	Vector2 eye_offset;
	for (int i = 0; i < models[cur_idx].shapes.size(); ++i)	{
		int is_eye = models[cur_idx].shapes[i].material.diffuseTexture;

		if (is_eye == 1 || is_eye == 4 || is_eye == 6 || is_eye == 8) {	
			switch (eye_status[cur_idx]) {
				case 0: eye_offset = Vector2(0.0f, 0.0f); break;
				case 1: eye_offset = Vector2(0.0f, 0.75f); break;
				case 2: eye_offset = Vector2(0.0f, 0.5f); break;
				case 3: eye_offset = Vector2(0.0f, 0.25f); break;
				case 4: eye_offset = Vector2(0.5f, 0.0f); break;
				case 5: eye_offset = Vector2(0.5f, 0.75f); break;
				case 6: eye_offset = Vector2(0.5f, 0.5f); break;
				default: break;
			}
		}
		else
			eye_offset = Vector2(0.0f, 0.0f);
		
		temp_info = glGetUniformLocation(program, "eye_offset");
		glUniform2fv(temp_info, 1, &(eye_offset[0]));
		
		temp_info = glGetUniformLocation(program, "Ka");
		glUniform3fv(temp_info, 1, &(models[cur_idx].shapes[i].material.Ka[0]));
		temp_info = glGetUniformLocation(program, "Kd");
		glUniform3fv(temp_info, 1, &(models[cur_idx].shapes[i].material.Kd[0]));
		temp_info = glGetUniformLocation(program, "Ks");
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		case GLFW_KEY_ESCAPE:
			...
			break;
		case GLFW_KEY_Z:
			cur_idx = (++cur_idx) % model_list.size();
			break;
		case GLFW_KEY_X:
			cur_idx = (--cur_idx) >= 0 ? cur_idx % model_list.size() : model_list.size() - 1;
			break;
		case GLFW_KEY_O:
			if (cur_proj_mode == Perspective)
			{
		...
		case GLFW_KEY_J:
			...
			break;
		case GLFW_KEY_RIGHT:
			eye_status[cur_idx] = (++eye_status[cur_idx]) % 7;
			break;
		case GLFW_KEY_LEFT:
			eye_status[cur_idx] = (--eye_status[cur_idx]) >= 0 ? eye_status[cur_idx] : 6;
			break;
		case GLFW_KEY_G:
			if (mag_mode == GL_NEAREST)
				mag_mode = GL_LINEAR;
			else
				mag_mode = GL_NEAREST;
			break;
		case GLFW_KEY_B:
			if (min_mode == GL_NEAREST)
				min_mode = GL_LINEAR_MIPMAP_LINEAR;
			else
				min_mode = GL_NEAREST;
			break;
		default:
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case LightEdit:
		if (light_src == DIRECTIONAL) {
			light_info[0].diffuse += Vector3(diff, diff, diff);
			cout << "(LightEdit) DirLight diffuse -> " << light_info[0].diffuse << "\n";
		}
		else if (light_src == POINT) {
			light_info[1].diffuse += Vector3(diff, diff, diff);
			cout << "(LightEdit) PointLight diffuse -> " << light_info[1].diffuse << "\n";
		}
		else if (light_src == SPOT) {
			light_info[2].spot_cutoff += yoffset / scaling_factor * 10.0f;
			...
			cout << "(LightEdit) SpotLight Cutoff -> " << light_info[2].spot_cutoff << "\n";
		}
		break;
	case ShininessEdit:
		shininess += yoffset;
...
static void cursor_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	if (mouse_pressed) {
		if (starting_press_x < 0 || starting_press_y < 0) {
			...
			case LightEdit:
				if (light_src == DIRECTIONAL) {
					light_info[0].position.x += diff_x / scaling_factor;
					light_info[0].position.y -= diff_y / scaling_factor;
					cout << "(LightEdit) Directional light position: ( " << light_info[0].position.x << " , " <<  light_info[0].position.y << " , " << light_info[0].position.z << " )" << endl;
				}
				else if (light_src == POINT) {
					light_info[1].position.x += diff_x / scaling_factor;
					light_info[1].position.y -= diff_y / scaling_factor;
					cout << "(LightEdit) Point light position: ( "  << light_info[1].position.x << " , " << light_info[1].position.y << " , " << light_info[1].position.z << " )" << endl;
				}
				else if (light_src == SPOT) {
					light_info[2].position.x += diff_x / scaling_factor;
					light_info[2].position.y -= diff_y / scaling_factor;
					cout << "(LightEdit) Spot light position: ( " <<  light_info[2].position.x << " , " << light_info[2].position.y << " , " << light_info[2].position.z << " )" << endl;
				}
			break;
			}
		}
...
void initParameter()
{
	...
	light_info[2].spot_cutoff = 30.0f;
    light_info[2].spot_exp = 50.0f;
...
{	
	// AngleGet = glGetUniformLocation(program, "angle");
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "111061553 HW3", NULL, NULL);
    if (window == NULL)
    {
